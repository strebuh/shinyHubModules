  ...
  tagList(
    fluidPage(
      sidebarLayout(
        ...
        mainPanel(
          # textOutput(ns("inputs_list"))
          plotly::plotlyOutput(ns("radar_plot"), height="400") # %>%  shinycssloaders::withSpinner(color = "#3F3138", type = 6)
        )
      )
    )
  )
...
compareRadarServer <- function(id, dataset, categ_vars#,
                               # id_var,
                               # grouper,
                               # mapped_vars,
                               # aggregation
...
ui <- fluidPage(
  compareRadarUI("USA_data", mapped_vars = c("Population", "Income","Illiteracy", "Life Exp", "Murder", "HS Grad", "Frost", "Area"))
)

server <- function(input, output, session) {
  compareRadarServer("USA_data", compareR::usa_data, categ_vars=c("region", "rand_status", "rand_class"))
}
